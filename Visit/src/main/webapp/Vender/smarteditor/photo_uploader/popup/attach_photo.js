/// <reference path="photo_uploader.html" />
/// <reference path="jindo.fileuploader.js" />
/// <reference path="FileUploader_html5.aspx" />
/// <reference path="FileUploader_html5.aspx" />
	//변수 선언 및 초기화
	var nImageInfoCnt = 0;
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("이미지파일 (jpg,jpeg,gif,png,bmp)만 업로드 가능합니다.");
    ...
    function html5Upload() {	
    	...
    	//sUploadURL = 'FileUploader_html5.aspx';  	//upload URL
    	sUploadURL = '/Vender/smarteditor/photo_uploader/popup/FileUploader_html5.aspx';
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("이미지 파일(jpg,jpeg,gif,png,bmp)만 업로드 하실 수 있습니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (e){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/FileUploader.aspx',	// 업로드 URL입니다.
